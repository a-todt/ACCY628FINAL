...
import { ROLE_LABELS, canViewAuditLog } from "@/lib/roles";
...
export default function AuditLogPage() {
  ...
  const allowed = canViewAuditLog(effectiveRole);
  ...
  const load = useCallback(async () => {
    ...
    const [auditRes, profilesRes] = await Promise.all([
      supabase.from("access_audit_log").select("*").order("created_at", { ascending: false }).limit(2000),
  ...
  const onExport = () => {
    ...
    downloadXlsx(`audit-log-${stamp}.xlsx`, exportRows, "Audit Log");
  };

  if (!allowed) {
    return (
      <AlertBanner type="error">
        Access denied. Only admins can view the system audit log.
      </AlertBanner>
    );
  }
  ...
  return (
    <div>
      <PageHeader
        title="Audit Log"
        subtitle="Complete record of system edits with user, permission level, and related records."
        actions={
