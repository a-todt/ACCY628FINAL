  ...
  const exportCsv = () => {
    const exportRows = [
      ...sortedRows.map(({ project, calcs, health }) => ({
        ...
        "Contract Value": Math.round(colNum(project, P.contractValue)),
        "Estimated Total Cost": Math.round(colNum(project, P.estimatedCost)),
        "Costs to Date": Math.round(calcs.actualCostsToDate),
        "Completion %": Number(calcs.completionPercentage.toFixed(1)),
        "Revenue Earned": Math.round(calcs.revenueEarned),
        "Billed to Date": Math.round(calcs.billedToDate),
        Overbilling: Math.round(calcs.overbilling),
        Underbilling: Math.round(calcs.underbilling),
        "Retainage Receivable": Math.round(calcs.retainageHeld),
        "Projected Profit": Math.round(calcs.projectedProfit),
        "Projected Margin %": Number(calcs.projectedMargin.toFixed(1)),
        Health: health,
      })),
      {
        Project: "TOTALS",
        "Contract Value": Math.round(totals.contractValue),
        "Estimated Total Cost": Math.round(totals.estimatedCost),
        "Costs to Date": Math.round(totals.costsToDate),
        "Completion %": Number(totalsCompletion.toFixed(1)),
        "Revenue Earned": Math.round(totals.revenueEarned),
        "Billed to Date": Math.round(totals.billedToDate),
        Overbilling: Math.round(totalsOverbilling),
        Underbilling: Math.round(totalsUnderbilling),
        "Retainage Receivable": Math.round(totals.retainageHeld),
        "Projected Profit": Math.round(totalsProjectedProfit),
