 ...
 *
 * At or under $250,000: Project Manager or Accounting may approve.
 * Over $250,000: Accounting or Admin / Owner only (PM cannot approve).
 *
 * Pending COs are excluded from revised contract value / billing basis until approved.
 *
 * Role mapping (demo labels): owner → Accounting, admin → Admin / Owner.
 */
...
/** Accounting uses the `owner` role in the current demo role model. */
export function isAccountingApprover(role: UserRole): boolean {
  return role === "owner";
}

/** Admin / Owner uses the `admin` role. */
